...
export const registerLiveSocketHandler = (io: Server, socket: Socket): void => {
  ...
  socket.on('join_live_room', ({ eventId }: { eventId: string }) => {
    ...
    let viewersSet = roomViewerCounts.get(eventId);
    if (!viewersSet) {
      viewersSet = new Set();
      roomViewerCounts.set(eventId, viewersSet);
    }
    ...
    logger.info(
      `[LiveSocket] Socket ${socket.id} (User: ${userId}) joined room ${roomName}. Total viewers: ${count}`,
    );
